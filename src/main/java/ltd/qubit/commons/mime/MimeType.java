...
public final class MimeType implements Serializable, CloneableEx<MimeType> {
  ...
  public boolean matches(final byte[] buffer, final int n) {
    ...
    for (final String parentName : superTypes) {
      ...
      if ((parent != null) && parent.matches(buffer, n)) {
        return true;
      }
    }
    return false;
  ...
  MimeMagic getMatchedMagic(final byte[] buffer, final int n, final int bestPriority) {
    int best = bestPriority;
    if ((magics != null) && (! magics.isEmpty())) {
      MimeMagic result = null;
      for (final MimeMagic magic : magics) {
        final int priority = magic.getPriority();
        if ((priority >= best) && magic.matches(buffer, n)) {
          result = magic;
          best = priority;
        }
      }
      ...
      for (final String parentName : superTypes) {
        final MimeType parent = repository.get(parentName);
        if (parent != null) {
          final MimeMagic magic = parent.getMatchedMagic(buffer, n, best);
          if (magic != null) {
            result = magic;
            best = magic.getPriority();
          }
        }
      }
      return result;
