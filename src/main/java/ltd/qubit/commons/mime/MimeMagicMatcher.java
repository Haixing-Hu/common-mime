...
public final class MimeMagicMatcher implements Serializable, CloneableEx<MimeMagicMatcher> {
  ...
  public boolean matches(final byte[] buffer, final int n) {
    ...
    switch (type) {
      case TYPE_STRING:
        if (! matchesString(buffer, n)) {
          return false;
        }
        break;
      case TYPE_HOST16:
        reverseOrder = (ByteOrder.nativeOrder() != ByteOrder.BIG_ENDIAN);
        if (!matchesInt16(buffer, n, reverseOrder)) {
          return false;
        }
        break;
      case TYPE_HOST32:
        reverseOrder = (ByteOrder.nativeOrder() != ByteOrder.BIG_ENDIAN);
        if (!matchesInt32(buffer, n, reverseOrder)) {
          return false;
        }
        break;
      case TYPE_BIG16:
        if (!matchesInt16(buffer, n, false)) {
          return false;
        }
        break;
      case TYPE_BIG32:
        if (!matchesInt32(buffer, n, false)) {
          return false;
        }
        break;
      case TYPE_LITTLE16:
        if (!matchesInt16(buffer, n, true)) {
          return false;
        }
        break;
      case TYPE_LITTLE32:
        if (!matchesInt32(buffer, n, true)) {
          return false;
        }
        break;
      case TYPE_BYTE:
        if (!matchesByte(buffer, n)) {
          return false;
        }
        break;
      default:
  ...
  private boolean matchesByte(final byte[] buffer, final int n) {
    if ((mask == null) || (mask.length == 0)) {
      final byte v = value[0];
      final int end = (offsetEnd < n ? offsetEnd : n - 1);
  ...
  private boolean matchesString(final byte[] buffer, final int n) {
    if ((mask == null) || (mask.length == 0)) {
      int end = n - value.length;
      if (end > offsetEnd) {
        end = offsetEnd;
      }
      for (int offset = offsetBegin; offset <= end; ++offset) {
  ...
  private boolean matchesInt16(final byte[] buffer, final int n, final boolean reverseOrder) {
    if ((mask == null) || (mask.length == 0)) {
      final byte v0;
      final byte v1;
      if (reverseOrder) {
  ...
  private boolean matchesInt32(final byte[] buffer, final int n, final boolean reverseOrder) {
    if ((mask == null) || (mask.length == 0)) {
      final byte v0;
      final byte v1;
      final byte v2;
